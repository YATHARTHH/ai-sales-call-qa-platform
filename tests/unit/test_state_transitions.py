...
def test_gate_valid_and_invalid_transitions():
    ...
    StateTransitionPolicy.validate_gate_transition(GateStatus.PENDING, GateStatus.HELD)
    # Allowed: HELD -> PASSED (Team Lead override)
    StateTransitionPolicy.validate_gate_transition(GateStatus.HELD, GateStatus.PASSED)

    # Illegal: PASSED cannot transition back to HELD
    with pytest.raises(StateTransitionError):
        StateTransitionPolicy.validate_gate_transition(GateStatus.PASSED, GateStatus.HELD)
